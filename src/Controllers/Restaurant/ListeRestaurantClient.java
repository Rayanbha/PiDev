...
public class ListeRestaurantClient {
    ...
    private final RestaurantService service = new RestaurantService();
        private user user;
    public void InitUser(int user1)
    {
        UserService us=new UserService();
        this.user=us.findcin(user1);
        System.out.println("USSERRR"+user);
    }
    @FXML
    ...
    void reserver(ActionEvent event) {
        ...
        if(selectedresto != null) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/UI/AfficherTable.fxml"));
                Parent root = loader.load();
                AfficherTable controller = loader.getController();
                controller.InitResto(selectedresto,user); // Pass selected restaurant data to AfficherTable
                Scene scene = new Scene(root);
                Stage stage = new Stage();
                stage.setScene(scene);
                stage.show();
